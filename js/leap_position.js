...
function mainLeap() {
  ...
  function updateLoop(frame) {
    for (let h = 0; h < MAX_HANDS; h++) {
      if (h < frame.hands.length) {
        const leapHand = frame.hands[h];

        if (leapHand.valid) {
          showHand(h);
          updateHandPosition(h, leapHand);
          updateHandLabelText(h, leapHand);
          // checkAudio();
        } else {
          hideHand(h);
          // stopAudio();
  ...
  function showHand(h) {
    // $("#palm_"+h).show();
    // $('#label_'+h+'_palm').show();
    for (let f = FINGER_MIN; f < FINGER_COUNT; f++) {
      // $("#mcp_"+h+"_"+f).show();
      // $("#pip_"+h+"_"+f).show();
      $(`#dip_${h}_${f}`).show();
  ...
  function updateHandPosition(h, leapHand) {
    // const palmPosition = normalizeVector(leapHand.palmPosition).multiplyScalar(normalToScreen);
    ...
    // $("#palm_"+h).css('position', 'absolute');
    for (let f = FINGER_MIN; f < FINGER_COUNT; f++) {
      // var mcpPosition = normalizeVector(leapHand.fingers[f].mcpPosition).multiplyScalar(normalToScreen);
      // var pipPosition = normalizeVector(leapHand.fingers[f].pipPosition).multiplyScalar(normalToScreen);
      const dipPosition = normalizeVector(leapHand.fingers[f].dipPosition).multiplyScalar(normalToScreen);
      const labelPosition = dipPosition;
  ...
  function updateHandLabelText(h, leapHand) {
    // $('#label_'+h+'_palm').text(leapHand.type);
    for (let f = FINGER_MIN; f < FINGER_COUNT; f++) {
      const finger = leapHand.fingers[f];
  ...
  function normalizeVector(position) {
    const pos = new Vector(position[0] / 10.0, position[1] / 10.0, position[2] / 10.0);
